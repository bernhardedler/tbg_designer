...
public class ZaehlerverteilerNapMitte {
	...
	public static final String TEMPLATE_XL = "zaehlerverteiler_nap_mitte_xl";
	public static final String TEMPLATE_XXL = "zaehlerverteiler_nap_mitte_xxl";
	...
	public static void createAll() throws IOException {
		...
		for (String color : Icons.colors.keySet()) {
			...
			tmpXml = Icons.readTemplateFromFile(TEMPLATE_XL)
					.replaceAll(Icons.COLOR, String.valueOf(Icons.colors.get(color)));
			tmpFilePath = subfolder + "zaehlerverteiler_" + color + "_nap_mitte_xl.svg";
			Icons.writeIconToFile(tmpFilePath, tmpXml);
			tmpXml = Icons.readTemplateFromFile(TEMPLATE_XXL)
					.replaceAll(Icons.COLOR, String.valueOf(Icons.colors.get(color)));
			tmpFilePath = subfolder + "zaehlerverteiler_" + color + "_nap_mitte_xxl.svg";
			Icons.writeIconToFile(tmpFilePath, tmpXml);
		}
	}
}
