...
public class TBGResetOriginModelChangeOperation extends AbstractModelChangeOperation<Void> {
	...
	/**
	 * Constructor to perform this operation by using the displayed figures bounds
	 * of this diagram.
	 * 
	 * @param diagramEditPart the root Diagram EditPart.
	 */
	public TBGResetOriginModelChangeOperation(DiagramEditPart diagramEditPart) {
	 ...
	 * @param diagramEditPart the root Diagram EditPart.
	 */
	public TBGResetOriginModelChangeOperation(AbstractDiagramContainerEditPart containerEditPart) {
	...
	public Void execute() {
		...
		List<IGraphicalEditPart> primaryGraphicalEditParts = Lists
				.newArrayList(Iterables.filter(primaryEditParts, IGraphicalEditPart.class));
	 ...
	 * Some edges can be masked because of a scrollbar on a container. If one of the
	 * edge end is masked by the scroll size, the edge is masked too. The
	 * OrthogonalLayout doesn't relocate the masked edges and that causes wrong
	 * diagram bounds computation and persistent scroll bar if the old edge location
	 * is out of the current bounds.
	 */
			...
			if (!(editPart instanceof ConnectionEditPart)) {
				if (editPart.getModel() instanceof Node) {
					...
					if (constraint instanceof Location) {
						...
						((Node) editPart.getModel()).setLayoutConstraint(location);
					}
				}
			}
		}
	 ...
	 * 
	 * @param editParts given edit parts
	 ...
	 * visible anymore). We remove them from the list to compute the correct bounds.
	 * Note that some edges can be hidden due to a source or target which is no
	 * visible (because of scroll bar within a container for instance); in this case
	 * consider the edge as visible.
	 * 
	 * @param primaryEditParts the primary editParts list.
	 */
	...
	/**
	 * Gets the primary editparts on this surface, that is, the top-level shapes and
	 * connectors.
	 * 
	 ...
	 * @param defaultSize if there are no edit parts, the size of the bounding box
	 *                    will be the default one.
	 * @return the edit parts bounding box
	 */
	public static final Rectangle calculateBoundinBoxRelativeToContainer(List<IGraphicalEditPart> editparts,
			double frameSize, Dimension defaultSize) {
